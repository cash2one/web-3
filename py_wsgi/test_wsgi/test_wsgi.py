...
# @Last Modified time: 2016-11-29 09:35:31
#
...
'''
application()函数————符合WSGI标准的一个HTTP处理函数，它接收两个参数：
environ：一个包含所有HTTP请求信息的dict对象；
start_response：一个发送HTTP响应的函数，加上application函数返回值作为Body。


start_response()发送了HTTP响应的Header————只能发送一次，也就是只能调用一次start_response()函数
...
Web应用可以稍微改造一下，从environ里读取PATH_INFO，这样可以显示更加动态的内容：
