...
from login.forms.test_forms import MyForm, LoginForm
from login.models.test_models import EmailLogin
from login.common.send_email import return_a_code
    ...
    if request.method == 'GET':
        ...
        if request.GET.get("second") == "second":
            ...
            if fm_check.is_valid():
                ...
                return render(request, "test/form_with_style.html", {'form': fm})
            #
            # “提交的信息不符合要求”时被调用
            return render(request, "test/form_test.html", {'form': fm_check})
        #
        # “首次访问”时被调用
        return render(request, 'test/form_test.html', {'form': fm})
    else:
        #
        # form表单绑定接收的数据，进行数据验证，反馈错误提示信息
        ...
        #
        # “提交的信息不符合要求”时被调用
        return render(request, "test/form_with_style.html", {'form': fm_check})
    ...
    def get(self, request, *args, **kwargs):
        ...
        if request.GET.get("not_first") == "not_first":
            ...
            if fm_check.is_valid():
                ...
                return render(request, "test/login.html", {'form': fm_check})
        return render(request, "test/login.html", {'form': fm})
