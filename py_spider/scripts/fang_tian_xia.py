...
bj_districts = ["a01", "a00", "a02", "a03", "a06", "a07", "a0585", "a011", "a010",
                "a012", "a09", "a08", "a013", "a016", "a014", "a015", "a0987", "a011817"]
sh_districts = ["a025", "a019", "a020", "a027", "a028", "a024", "a021", "a018", "a023",
                "a022", "a026", "a029", "a030", "a0586", "a031", "a032", "a035", "a0996", "a01046"]

for i in bj_districts:
    # i = "a01"
    # if i:
...
def process(p):
    ...
    if p.isDefaultLevel():
        ...
        """
        page_div = soup.findAll(name="div", attrs={"id": "esf_B03_04"})
        if page_div:
            all = page_div[0].findAll('a')
            for a in all:
                url = a.get("href")
                if url:
        ...
        if not address:
            address = xpath(
                "//dl[2]/dt[2]").text().strip("<dt>/").strip().split(" ")[-1]

        floor = xpath("//dl[2]/dd[1]/text()").text().strip()
        fee = xpath("//dl[2]/dd[2]/text()").text().strip()

        fee_company = xpath(
            "//dl[2]/dd[3]").text().strip("<dd>/").strip().split(" ")[-1]
        office_level = xpath(
            "//dl[2]/dd[4]").text().strip("<dd>/").strip().split(" ")[-1]
        zhuangxiu = xpath("//dl[2]/dd[5]").text().strip("<dd>/").strip().split(
            ">")[-1].replace("\r\n", "").replace(" ", "")
        type = xpath(
            "//dl[2]/dd[6]").text().strip("<dd>/").strip().split(" ")[-1]
