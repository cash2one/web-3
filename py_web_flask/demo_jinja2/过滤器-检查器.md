...
| abs(number)           | 返回数字的绝对值                                |
|-----------------------|-------------------------------------------------|
| batch(value,          | 将序列value分割成一堆list                       |
| linecount,            | list个数                                        |
| fill_with=None)       | fill_with，长度不足时的默认值                   |
|-----------------------|-------------------------------------------------|
| capitalize(s)         | 首字符大写                                      |
|-----------------------|-------------------------------------------------|
| center(value,         | 将value居中                                     |
| width=80)             | 长度为width，左右补' '                          |
|-----------------------|-------------------------------------------------|
| default(value,        | default————别名是d                              |
| default_value=u'',    | 如果value未定义，则显示default_value            |
| boolean=False)        | bool意义未知                                    |
|-----------------------|-------------------------------------------------|
| dictsort(value,       | 字典排序                                        |
| case_sensitive=False, | case_sensitive，是否大小写敏感                  |
...
| first(seq)            | 返回序列的第一个值                              |
|-----------------------|-------------------------------------------------|
| float(value,          | 将一个值转换成浮点数                            |
| default=0.0)          | 如果转换失败则返回default                       |
|-----------------------|-------------------------------------------------|
| forceescape(value)    | 不管value是否被转义过，一律进行html转义         |
|                       | 可能会发生二次转码                              |
|-----------------------|-------------------------------------------------|
| format(value,         | 等同于"%s,%s" % (str1, str2)                    |
| *args, **kwargs)      |                                                 |
|-----------------------|-------------------------------------------------|
| groupby(value,        | 类似SQL的group by，将一个序列里的对象/字典分组  |
| attribute)            | 按照attribute分组                               |
