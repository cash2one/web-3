###FLASK
- 框架本身只包含路由分发，请求和响应的封装和插件系统
- （作者开发）Werkzeug(处理WSGI)、jinja2和Flask-SQLAlchemy（Model层）
- 轻巧、灵活，靠插件进行扩展，插件丰富
- 适合开发没有复杂的分层和逻辑的REST API的系统
- 或者需要自己重新实现大部分插件内容一个大型的系统
...
|                     | 支持运行时切换语言，并且语言切换线程独立（用户可以设置自己的界面语言） |
|---------------------|------------------------------------------------------------------------|
